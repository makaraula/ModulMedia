...
if(! defined('WATERMARK_IMAGE_ENABLED'))	define ('WATERMARK_IMAGE_ENABLED', false);				// Watermark
...
if(! defined('DEFAULT_WMI') ) 			define ('DEFAULT_WMI', $_SERVER["DOCUMENT_ROOT"].'VirtusAdmin/images/watermark.png|c|60');
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$watermark_image_details = $this->param('wmi', DEFAULT_WMI);
		...
		// watermark image
		if ($watermark_image_details && WATERMARK_IMAGE_ENABLED)
		{
			@list( $watermark_image, $watermark_position, $watermark_opacity ) = explode('|', $watermark_image_details);
			...
			$watermark_extension = pathinfo($watermark_image, PATHINFO_EXTENSION);
			if($watermark_extension != 'png') die('Watermark image must be ".png" please convert your watermark image to png.');
			$watermark_image = dirname(__FILE__) . DIRECTORY_SEPARATOR . $watermark_image;
			if(!file_exists($watermark_image)) die('Can not found watermark image.');
		
			$watermark_image = imagecreatefrompng( $watermark_image );
			$watermark_width = imagesx( $watermark_image );
			$watermark_height = imagesy( $watermark_image );
		
			if(strtolower($watermark_position) == 'c'){
				$watermark_positionx = $new_width/2 - $watermark_width/2;
				$watermark_positiony = $new_height/2 - $watermark_height/2;
			}else{
				@list($watermark_positionx, $watermark_positiony ) = explode('x', $watermark_position);
				if(is_numeric($watermark_positionx) === false or is_numeric($watermark_positiony) === false){
					die('Watermark position could be \'C\' (center) or \'25x25\'');
				}
			}
		
			$watermark_opacity = intval($watermark_opacity);
			if($watermark_opacity === 0 ) $watermark_opacity = 80;
		
			imagealphablending( $canvas, true );
			imagecopymerge_alpha( $canvas, $watermark_image, $watermark_positionx, $watermark_positiony, 0, 0, $watermark_width, $watermark_height, $watermark_opacity);
			imagealphablending($canvas, false );
			imagesavealpha($canvas , true );
		}
